...
test('the workbook round-trips with every structural promise kept', async () => {
  const buf = await buildCasWorkbook({ client: 'Hartley Construction Ltd', rows });
  ...
  assert.equal(typeof ws.autoFilter === 'string' ? ws.autoFilter : (ws.autoFilter && ws.autoFilter.to ? 'obj' : ''),
    typeof ws.autoFilter === 'string' ? 'A1:J' + (rows.length + 1) : 'obj');
  ...
  const dv = ws.getRow(2).getCell('I').dataValidation;
